...
> 为什么从 28 开始？因为第 27 课刚把 RAG 包成了 Express API。
...
| # | 主题 | 新增能力 | 关键 API / 概念 |
|---|------|---------|----------------|
| 28 | 威胁模型总览 | **建立 AI 工程安全心智模型** | HTML 画四大风险面（无代码，类似第 8/9/22 课） |
| 29 | API Key 卫生 | **Key 只在服务端、配额隔离、不进 Git** | `.env` · project-scoped keys · usage limits |
| 30 | Prompt 注入实验室 | **亲眼看到模型被带偏** | 5 种攻击 payload 跑同一个不设防的 system prompt |
| 31 | 输入侧加固 | **role 隔离 + 定界符 + 长度/白名单** | `instructions` 字段 · `<user_input>` 包裹 · `max_output_tokens` |
| 32 | Moderation API | **免费双向审核** | `client.moderations.create` · 13 个 category + score 阈值 |
| 33 | 输出契约 | **只能返回合法结构** | `response_format` JSON Schema `strict: true` + Zod |
| 34 | 限流与重试 | **读懂 429，写出能活过尖峰的客户端** | `x-ratelimit-*` headers · 指数退避 + jitter · `maxRetries` |
| 35 | 超时与降级 | **可失败，不是挂掉** | `AbortController` · timeout 分层 · 模型降级 · `service_tier` |
| 36 | 日志与审计 | **出了事查得到** | `response.id` · `response.usage` · `store` 开关 · NDJSON 审计日志 |
| 37 | 安全硬化版 RAG | **把 28–36 全部叠加到第 27 课上** | 7 层护栏 + 跑第 30 课 5 个攻击 payload 全部失败 |

**两种视角覆盖**：
- **防御视角**（28/30/31/32/33）：Prompt 注入、有害内容、输出越界。
- **运维视角**（29/34/35/36）：Key、限流、超时、日志。
- **整合视角**（37）：把所有东西叠在一个真实的 RAG 服务上。
...
```
28-threat-model-overview/
└─ unsafe-server.ts       # 故意：Key 硬编码 · system prompt 拼接用户输入 · 无审核 · 无超时

_shared/
├─ attack-payloads.ts     # 5 条攻击输入，30 课和 37 课都会用
...
### 第 28 课 · `28-threat-model-overview/`
...
### 第 30 课 · `30-prompt-injection-basics/`
**目标**：亲眼看到模型被带偏。

**`attack-lab.ts`**：
- 用第 28 课的 `unsafe-server` 启动
...
- 再跑一次第 30 课的 5 条 payload，预期**掉到 1~2 条中招**

README 强调：
> **输入加固不是银弹**，它让简单攻击失败，让复杂攻击更显眼（因为被迫用更长的模板）。真正的兜底在第 32 课（审核）和第 33 课（结构化输出）。

### 第 32 课 · `32-moderation-api/`
...
### 第 34 课 · `34-rate-limits-and-retry/`
...
### 第 35 课 · `35-timeouts-degradation/`
**目标**：可失败，不是挂掉。
...
### 第 37 课 · `37-safety-hardened-rag/`
**目标**：收束实战。把 28–36 全部叠加到第 27 课的 RAG Express 项目上。

**`server.ts`** 的请求流水线：
```
POST /api/rag
  → [29] 读 .env，校验 Key
  → [36] 生成 requestId，开始 audit
  → [31] 输入加固：长度 / 定界符 / 控制字符
  → [32] Moderation 输入审
  → [26/33] Responses API + file_search tool + structured output
  → [34] 内置 retry wrapper
  → [35] 3 秒超时 + 降级链
  → [32] Moderation 输出审
  → [36] 写 audit log
...
**验收测试**：跑第 30 课的 5 条 attack payload，**全部必须被挡下**（或者得到结构化 `refused: true`）。
...
34-rate-limits-and-retry/
├─ retry.ts
└─ README.md

35-timeouts-degradation/
├─ degrade.ts
└─ README.md
...
{
  "scripts": {
    "l28": "tsx 28-threat-model-overview/unsafe-server.ts",
    "l29": "tsx 29-api-key-hygiene/server.ts",
    "l30": "tsx 30-prompt-injection-basics/attack-lab.ts",
    "l31": "tsx 31-input-hardening/harden-input.ts",
    "l32": "tsx 32-moderation-api/moderate.ts",
    "l33": "tsx 33-output-contract/contract.ts",
    "l34": "tsx 34-rate-limits-and-retry/retry.ts",
    "l35": "tsx 35-timeouts-degradation/degrade.ts",
    "l36": "tsx 36-logging-and-audit/audit.ts",
    "l37": "tsx 37-safety-hardened-rag/server.ts"
  }
}
```
...
```
OPENAI_API_KEY=sk-...
VECTOR_STORE_ID=vs_xxx   # 第 37 课用
```
...
1. **Step A** 写 `_shared/attack-payloads.ts` + `_shared/kb-secret.md`（给 30/37 课用）
2. **Step B** 写 `28-threat-model-overview/{threat-model.html, unsafe-server.ts}` + README
3. **Step C** 写 `29-api-key-hygiene/server.ts` + `.env.example` + README
4. **Step D** 写 `30-prompt-injection-basics/attack-lab.ts` + README（用 Step B 的 unsafe-server 当靶子）
5. **Step E** 写 `31-input-hardening/harden-input.ts` + README
6. **Step F** 写 `32-moderation-api/moderate.ts` + README
7. **Step G** 写 `33-output-contract/contract.ts` + README
8. **Step H** 写 `34-rate-limits-and-retry/retry.ts` + README
9. **Step I** 写 `35-timeouts-degradation/degrade.ts` + README
10. **Step J** 写 `36-logging-and-audit/audit.ts` + README
11. **Step K** 写 `37-safety-hardened-rag/server.ts` + README（叠加所有前面能力到第 27 课 RAG 上）
12. **Step L** 更新根 `README.md` 加 28~37 行 & `package.json` 加 scripts
13. **Step M** `pnpm typecheck` 全绿

**验证策略**：
- Typecheck：全程保证 `pnpm typecheck` 通过
- 运行时：所有课都需要真 `OPENAI_API_KEY` + 网络；Moderation 免费所以不额外计费
- **关键验收**：`pnpm l30` 在第 28 课靶子上跑，应看到被带偏；同样的 payload 打到 `pnpm l37`（硬化版）应全部被挡下 —— 这是整个模块最直观的教学闭环
...
| 把 RAG chunk 直接拼进 prompt | 间接注入风险：文档里也可能藏"忽略前面规则"。第 31 课的 `<context>` 定界符 + 第 33 课的 structured output 两层兜底 |
...
**28–37**：**工程护栏**（威胁建模 / Key / 注入 / 审核 / 结构化 / 限流 / 超时 / 日志 / 综合硬化）← 本篇

教学闭环：从"能跑"（01）→"能用"（14）→"能记"（21）→"能查资料"（27）→"**能上线**"（37）。
